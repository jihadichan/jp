...
- You have kanji which can have confs with one or more other kanji, e.g. `返 cf 反 cf 仮`

- A UberKanji card shows one kanji and must display all its confs additionally

- A conf for one kanji must be displayed on all other conf kanji, i.e. you have conf groups

- Confs are stored in a text file with special format (`反 cf 返 cf 仮`). 

  - There can be duplicates

  - No matter how the kanji are linked, as soon as they appear in one statement, it must be added to the group and become a conf for all other kanji, e.g.:
    ...
    But the resulting conf group must be: `反, 返, 仮`

- So you need:

  - A map of all kanji available which have confusions

  - This map must also contain the main reading and the meaning, e.g, `漬 (づ, pickled)`

  - And also a list of confusions to look them up
...
- The UberKanji card then takes the kanji of the card as key, gets the list of confusions and looks up every confusion to get their readings + meanings
