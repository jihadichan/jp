...
public class RomajiConverter {
    ...
    public static String fromKatakana(final String s) {
        ...
        for (int i = 0; i < s.length(); i++) {
            if (i <= s.length() - 2) {
                if (katakanaMap.containsKey(s.substring(i, i + 2))) {
                    ...
                    String value = s.substring(i + 1, i + 2);
                    t.append(katakanaMap.get(value).charAt(0));
                } else {
                    t.append(s.charAt(i));
                }
            } else {
    ...
    public static String fromHiragana(String hiragana) {
        return fromKatakana(JpHelpers.hiraToKata(hiragana));
    }
}
